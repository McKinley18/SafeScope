...
cases=(
  ...
  "no guardrail on mezzanine|Fall|high|false|1926.501"
...
for row in "${cases[@]}"; do
  ...
  family=$(echo $response | python3 -c "import sys, json; print(json.load(sys.stdin).get('classification', 'N/A'))")
  review=$(echo $response | python3 -c "import sys, json; print(str(json.load(sys.stdin).get('requiresHumanReview', 'N/A')).lower())")
  standards=$(echo $response | python3 -c "import sys, json; print(str(json.load(sys.stdin).get('suggestedStandards', [])))")
